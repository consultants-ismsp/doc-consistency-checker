...
| 검사 | 방식 | 내용 |
|---|---|---|
| 용어 표기 통일(모드 A) | 코드 | 같은 개념을 문서마다 다르게 표기하면 적발. 약어↔풀이·나열·등급은 거짓양성으로 거름 |
| 표준 용어 점검(모드 B) | LLM(옵션, 1회) | 용어사전 대비 비표준 표기 적발 |
| 수치 정합 | 코드 | 라벨 맥락 결합 자동 교차비교 + 룰 기반 검산/일치. 일반어(총계·합계·소계…)는 stoplist 제외 |
| 연계·종속·부분집합 | 코드 | 평가항목 ⊆ 법령매핑, 개선과제 ⊆ 수준평가 등 추적 룰 |
| 회사명 표기 통일 | 코드 | 수행사(고정) + 고객사(런타임 입력)가 문서 간 같은 표기인지 검사 |
| 오타 | LLM(옵션) | 추출 시 모은 오타 의심 표기 보고(회사명·참조문서 제외) |
