...
`,
  ...
  // 개선과제정의서: 개선과제가 대상으로 삼는 통제항목번호 + 종합표 건수 + 과제카드별 미흡건수.
  task_def: `아래는 ISMS-P '개선과제 정의서' 문서 블록입니다. 통제항목번호와 건수 수치를 원문 그대로 추출하세요.
...
추출 형식(JSON):
{
  ...
  "numbers": [
    {"key": "개선과제총건수", "value": 30, "source": {"section": "...", "para_index": 0, "snippet": "..."}}
  ],
  "term_hits": [
    {"value": "표현", "concept_hint": "개념", "source": {"section": "...", "para_index": 0, "snippet": "..."}}
  ]
...
규칙(통제항목): 대상 통제항목번호를 원문 그대로 나열하세요. 추론·집계하지 마세요. 없으면 비웁니다.

규칙(수치) — 아래 정규 key 이름을 '공백 없이' 그대로 사용하세요(있는 것만). value는 원문 숫자 그대로, 계산·합산 절대 금지:
- 종합표 서두 문장(예: "총 30건 ... 관리체계 개선 과제 19건 ... 자산취약점 개선 과제 11건")에서:
  - "개선과제총건수": 전체 개선과제 총 건수(예 30).
  - "관리체계개선과제건수": 관리체계 개선 과제 건수(예 19).
  - "자산취약점개선과제건수": 자산취약점 개선 과제 건수(예 11).
- 각 과제 카드의 "통합 미흡 사항: N종 / M건"에서 'M건'(미흡 건수)을, 그 카드가:
  - '위험도(평가값)'(관리체계 과제, M-xx)면 key "관리체계카드미흡건수" 로,
  - '위험도(SV)'(자산취약점 과제, T-xx)면 key "자산취약점카드미흡건수" 로,
  각 카드마다 한 건씩(같은 key 여러 개) 넣으세요. value는 그 카드의 M(건) 숫자만. '종' 숫자는 넣지 마세요.
